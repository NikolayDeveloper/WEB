...
{
    [TemplateFieldName(TemplateFieldName.Priority33WithoutCode)]
    internal class Priority33WithoutCode : TemplateFieldValueBase
    {
        public Priority33WithoutCode(IExecutor executor) : base(executor)
        {
        }
            ...
            switch (ownerType)
            {
                case Owner.Type.Request:
                    ...
                    var earlyRegs = request?.EarlyRegs
                        .Select(e => "(330) Двухбуквенный код по стандарту ВОИС ST .3 \r\n" + e.RegCountry?.Code)
                        .ToList();
                    result = string.Empty;
                    if (earlyRegs != null)
                    {
