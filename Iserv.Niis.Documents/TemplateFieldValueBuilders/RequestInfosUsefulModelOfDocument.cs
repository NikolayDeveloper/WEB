...
{
    [TemplateFieldName(TemplateFieldName.RequestInfosUsefulModelOfDocument_Complex)]
    internal class RequestInfosUsefulModelOfDocument : TemplateFieldValueBase
    {
        public RequestInfosUsefulModelOfDocument(IExecutor executor) : base(executor)
        {
        ...
        protected override dynamic GetInternal(Dictionary<string, object> parameters)
        {
            var requests = Executor.GetQuery<GetRequestsByDocumentIdQuery>()
                ...
                .Where(r => new[] { DicProtectionDocTypeCodes.RequestTypeUsefulModelCode }.Contains(r.ProtectionDocType?.Code ?? string.Empty));
