...
{
    [TemplateFieldName(TemplateFieldName.TransferDateWithCode)]
    internal class TransferDateWithCode : TemplateFieldValueBase
    {
        public TransferDateWithCode(IExecutor executor) : base(executor)
        {
        }
            ...
            switch (ownerType)
            {
                case Owner.Type.Request:
                    ...
                    var requestProtectionDoc = Executor.GetQuery<GetProtectionDocByRequestIdQuery>()
                        ...
                        .FirstOrDefault();
                    return $"(85) {requestProtectionDoc?.TransferDate?.ToTemplateDateFormat() ?? string.Empty}";
                case Owner.Type.ProtectionDoc:
                    var protectionDoc = Executor.GetQuery<GetProtectionDocByIdQuery>()
                        .Process(q => q.Execute((int)parameters["RequestId"]));
                    return $"(85) {protectionDoc?.TransferDate?.ToTemplateDateFormat() ?? string.Empty}";
                default:
                    return string.Empty;
            }
        }
