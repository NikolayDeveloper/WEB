...
@Injectable()
export class ProtectionDocSearchService extends BaseServiceWithPagination<ProtectionDocSearchDto> {
  private api = '/api/search/protectiondoc/';

  constructor(http: HttpClient,
    configService: ConfigService,
    errorHandlerService: ErrorHandlerService,
    private snackBarHelper: SnackBarHelper) {
    super(http, configService, errorHandlerService, '/api/search/protectiondoc/');
  }

  public get(): Observable<ProtectionDocSearchDto[]> {
    return this.http.get(`${this.configService.apiUrl}${this.api}`)
      ...
      .map((data: ProtectionDocSearchDto[]) => data);
  }
