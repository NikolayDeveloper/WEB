...
@Injectable()
export class DocumentSearchService extends BaseServiceWithPagination<DocumentSearchDto> {
  private api = '/api/search/document/';

  constructor(http: HttpClient,
    configService: ConfigService,
    errorHandlerService: ErrorHandlerService,
    private snackBarHelper: SnackBarHelper) {
    super(http, configService, errorHandlerService, '/api/search/document/');
  }

  public get(): Observable<DocumentSearchDto[]> {
    return this.http.get(`${this.configService.apiUrl}${this.api}`)
      ...
      .map((data: DocumentSearchDto[]) => data);
  }
