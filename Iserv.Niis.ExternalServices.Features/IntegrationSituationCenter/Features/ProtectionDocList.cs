...
namespace Iserv.Niis.ExternalServices.Features.IntegrationSituationCenter.Features
{
    ...
    {
        public class Query : IRequest<GetBtBasePatentListResult>
        {
            public readonly LogAction LogAction = new LogAction {Type = "GetBtBasePatentList"};
            public GetBtBasePatentListArgument Argument { get; set; }
            public GetBtBasePatentListResult Result { get; set; }
        }
        ...
        public class QueryHandler : IRequestHandler<Query, GetBtBasePatentListResult>
        {
            private readonly IProtectionDocService _protectionDocService;

            public QueryHandler(IProtectionDocService protectionDocService)
            {
                _protectionDocService = protectionDocService;
            }

            public GetBtBasePatentListResult Handle(Query message)
            {
                _protectionDocService.GetProtectionDocs(message.Argument, message.Result);
        ...
        public class QueryException : AbstractionExceptionHandler<Query, GetBtBasePatentListResult>
        {
            private readonly SystemInfoHelper _systemInfoHelper;
