...
@Injectable()
export class ContractSearchService extends BaseServiceWithPagination<ContractSearchDto> {
  private api = '/api/search/contract/';

  constructor(http: HttpClient,
    configService: ConfigService,
    errorHandlerService: ErrorHandlerService,
    private snackBarHelper: SnackBarHelper) {
    super(http, configService, errorHandlerService, '/api/search/contract/');
  }

  public get(): Observable<ContractSearchDto[]> {
    return this.http.get(`${this.configService.apiUrl}${this.api}`)
      ...
      .map((data: ContractSearchDto[]) => data);
  }
