...
namespace Iserv.Niis.Documents.TemplateFieldValueBuilders
{
    ...
    /// Патентообладатель
    /// </summary>
    [TemplateFieldName(TemplateFieldName.PatentOwnerKz)]
    internal class PatentOwnerKz : TemplateFieldValueBase
    {
        public PatentOwnerKz(IExecutor executor) : base(executor)
        {
        }
        ...
        {
            return customer => $"{customer.Customer.NameKz} ({customer.Customer.Country?.Code})";
        }


        private static Func<ProtectionDocCustomer, bool> IsProtectionDocCustomerPatentOwner()
        {
            return customer => customer.CustomerRole.Code == DicCustomerRoleCodes.PatentOwner;
        }
