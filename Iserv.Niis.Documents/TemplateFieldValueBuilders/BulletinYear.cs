...
{
    [TemplateFieldName(TemplateFieldName.BulletinYear)]
    internal class BulletinYear : TemplateFieldValueBase
    {
        public BulletinYear(IExecutor executor) : base(executor)
        {
        }
            ...
            switch (ownerType)
            {
                case Owner.Type.Request:
                    ...
                    var requestProtectionDoc = Executor.GetQuery<GetProtectionDocByRequestIdQuery>()
                        ...
                        .FirstOrDefault();
                    return requestProtectionDoc?.Bulletins?.FirstOrDefault()?.Bulletin?.PublishDate?.Year.ToString() ?? string.Empty;
                case Owner.Type.ProtectionDoc:
                    var protectionDoc = Executor.GetQuery<GetProtectionDocByIdQuery>()
                        .Process(q => q.Execute((int)parameters["RequestId"]));
                    return protectionDoc?.Bulletins?.FirstOrDefault()?.Bulletin?.PublishDate?.Year.ToString() ?? string.Empty;
                default:
                    return string.Empty;
            }
        }
