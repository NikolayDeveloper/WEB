...
namespace Iserv.Niis.BusinessLogic.Requests
{
    ...
    /// </summary>
    public class GetRequestByIdFromExportQuery : BaseQuery
    {
        /// <summary>
        /// Выполнение запроса.
        /// </summary>
        ...
        public async Task<Request> ExecuteAsync(int requestId)
        {
            ...
            var result = repository.AsQueryable()
                ...
                .Include(r => r.RequestCustomers).ThenInclude(c => c.Customer).ThenInclude(c => c.ContactInfos).ThenInclude(c => c.Type)
                ...
                .Include(r => r.ColorTzs).ThenInclude(d => d.ColorTz)
                ...
                .Include(r => r.Documents).ThenInclude(rd => rd.Document).ThenInclude(d => d.MainAttachment)
                .Include(r => r.Documents).ThenInclude(rd => rd.Document).ThenInclude(d => d.AdditionalAttachments)
