...
namespace Iserv.Niis.Documents.TemplateFieldValueBuilders
{
    ...
    /// Патентообладатель
    /// </summary>
    [TemplateFieldName(TemplateFieldName.PatentOwner)]
    internal class PatentOwner : TemplateFieldValueBase
    {
        public PatentOwner(IExecutor executor) : base(executor)
        {
        }
                ...
                case Owner.Type.ProtectionDoc:
                    ...
                    if (protectionDoc != null)
                    {
                        patentOwnerNames = protectionDoc.ProtectionDocCustomers
                            .Where(ProtectionDocHasPatentOwner())
        ...
        private static Func<RequestCustomer, bool> RequestHasPatentOwner()
        {
        ...
        {
            return customer => $"{customer.Customer.NameRu} ({customer.Customer.Country?.Code})";
        }


        private static Func<ProtectionDocCustomer, bool> ProtectionDocHasPatentOwner()
        {
            return customer => customer.CustomerRole.Code == DicCustomerRoleCodes.PatentOwner;
        }
