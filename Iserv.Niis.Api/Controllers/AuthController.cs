...
namespace Iserv.Niis.Api.Controllers
{
    ...
    [Route("api/Auth")]
    public class AuthController : BaseNiisApiController
    {
        ...
        private async Task<IActionResult> PasswordSignIn(LoginModel model)
        {
            ...
            //var userToVerify = Executor.GetQuery<GetUserByXinQuery>().Process(d => d.Execute(model.UserName));
            ...
            if (result.Succeeded)
            {
                ...
                await _signInManager.SignInAsync(userToVerify, true);
