...
{
    [TemplateFieldName(TemplateFieldName.BulletinDate)]
    internal class BulletinDate : TemplateFieldValueBase
    {
        public BulletinDate(IExecutor executor) : base(executor)
        {
        }
            ...
            switch (ownerType)
            {
                case Owner.Type.Request:
                    ...
                    var requestProtectionDoc = Executor.GetQuery<GetProtectionDocByRequestIdQuery>()
                        ...
                        .FirstOrDefault();
                    return requestProtectionDoc?.Bulletins?.FirstOrDefault()?.Bulletin?.PublishDate?.ToTemplateDateFormat() ?? string.Empty;
                case Owner.Type.ProtectionDoc:
                    var protectionDoc = Executor.GetQuery<GetProtectionDocByIdQuery>()
                        .Process(q => q.Execute((int)parameters["RequestId"]));
                    return protectionDoc?.Bulletins?.FirstOrDefault()?.Bulletin?.PublishDate?.ToTemplateDateFormat() ?? string.Empty;
                default:
                    return string.Empty;
            }
        }
