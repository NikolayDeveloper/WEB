...
{
    public abstract class NetCoreBaseParallelWorkflow<TWorkflowRequest, TEntity> where TWorkflowRequest : WorkflowRequest<TEntity> where TEntity : class, new()
    {
        ...
        protected abstract string[] CurrentWorkflowStageCodes { get; }


        protected NetCoreBaseParallelWorkflow()
        {
        ...
        public void Process()
        {
            var currentCodeWorkflowStages = WorkflowStageObjects.Where(s => s.IsCommonStage
                                                                || CurrentWorkflowStageCodes.Contains(s.CurrentWorkflowStageCode)
        ...
        private bool IsCurrentWorkflowStageCodeNotDefined => !CurrentWorkflowStageCodes.Any();
    }
}
