...
import { Component, EventEmitter, Input, OnDestroy, OnInit, Output, ViewChild, ChangeDetectorRef, AfterViewInit } from '@angular/core';
...
import { OperatorFor } from '../../../models/industrialdesign-dto.model';
...
const IndustrialdesignFieldConfigs: Map<ExpertSearchFieldEnum, FieldConfig> = new Map([
  ...
  [ExpertSearchFieldEnum.Icis, {
    enum: ExpertSearchFieldEnum.Icis,
    name: 'icis',
    label: 'МКПО'
  }],
...
@Component({
  selector: 'app-industrialdesign-form',
  templateUrl: './industrialdesign-form.component.html',
  styleUrls: ['./industrialdesign-form.component.scss']
})
export class IndustrialdesignFormComponent implements OnInit, OnDestroy, AfterViewInit {
  formGroup: FormGroup;
  CombineOperatorEnum = CombineOperatorEnum;
  ExpertSearchFieldEnum = ExpertSearchFieldEnum;
  industrialdesignFieldConfigs = IndustrialdesignFieldConfigs;
  configs: FieldConfig[];
  ...
  constructor(
    ...
    this.configs = Array.from(this.industrialdesignFieldConfigs.values());
    this.buildForm();
  }
  ...
  private buildQueryParamsFrom(formValues: any): any[] {
    ...
    if (formValues) {
      formValues.items.filter(item => item.value || item.subValue).forEach(item => {
        ...
        const fieldName = IndustrialdesignFieldConfigs.get(fieldSelector).name;
        let operator;
        if (item.value_mode) {
  ...
  private buildForm(values = []) {
    const formGroups = this.currentFields.map((currentField, index) => {
      const config = this.industrialdesignFieldConfigs.get(currentField.enum);
      const configGroup = {
