...
namespace Iserv.Niis.Infrastructure.Extensions.Filter
{
    public static class ResponseCollectionFilterExtensions
    {
        ...
        public static IQueryable<T> Sort<T>(this IQueryable<T> query, IQueryCollection queryCollection)
        {
            ...
            return query.Order(queryCollection);
        }
            ...
            IList<KeyValuePair<string, StringValues>> keyValuePairs)
        {
            try
            {
                ...
                var filterValuePairs = keyValuePairs.Where(p => !new[] {"_sort", "_order"}.Contains(p.Key))
                    .Where(p => !p.Key.Contains(Operators.FilterFields)).ToList();
                var filterFields = keyValuePairs.Where(p => p.Key.Contains(Operators.FilterFields)).ToList();
                ...
                foreach (var pair in filterValuePairs)
                {
                    ...
                    if (filterFields.Any() && operatorCode == "_q")
                    {
                        var fields = filterFields.SelectMany(f => f.Value.Select(v => v.ToLower())).ToList();

                        if (extractedValue.Contains(';') || extractedValue.Contains(','))
                        {
                            string[] extractedValues = extractedValue.ToString().Split(';', ',');
                            expression = BuildFullTextSearchExpressionForTheSelectedFields<T>(parameter, extractedValues, operatorCode, fields);
                        } else expression = BuildFullTextSearchExpressionForTheSelectedFields<T>(parameter, extractedValue, operatorCode, fields);
                    }
                    else if (operatorCode == "_q")
                    {
                        if (extractedValue.Contains(';') || extractedValue.Contains(','))
                        {
                            string[] extractedValues = extractedValue.ToString().Split(';', ',');
                            expression = BuildExpression<T>(parameter, split.fieldName, operatorCode, extractedValues);
                        } else expression = BuildExpression<T>(parameter, split.fieldName, operatorCode, extractedValue);
                    }
                    else
                    {
						if(extractedValue.Contains(';') || extractedValue.Contains(','))
        ...
        private static Expression BuildExpression<T>(ParameterExpression parameter, string fieldName,
           string operatorCode, string[] extractedValues)
        {
            var expressionFunc = Operators.GetExpressionFunc(operatorCode);
            ...
            {
                case Operators.Type.FullTextSearch:
                    return BuildFullTextSearchExpression<T>(parameter, extractedValues, expressionFunc);
                case Operators.Type.In:
                    return BuildIncludeExpression<T>(parameter, fieldName, extractedValues);
                case Operators.Type.Contains:
                    return BuildContainsExpression<T>(parameter, fieldName, extractedValues, expressionFunc);
                case Operators.Type.ContainsDateRange:
                    return BuildContainsRangeExpression<T>(parameter, fieldName, extractedValues, expressionFunc);
                default:
                    var property = GetPropertyInfo<T>(fieldName, operatorCode);
                    return expressionFunc.Invoke(
                        Expression.MakeMemberAccess(parameter, property),
                        BuildNullableConstantExpression<T>(extractedValues, property));
            }
        }
        ...
        private static Expression BuildContainsExpression<T>(ParameterExpression parameter, string fieldName,
            string[] extractedValues, Func<MemberExpression, Expression, Expression> expressionFunc)
        {
            var propertyInfo = GetPropertyInfo<T>(fieldName);

            var expressions = extractedValues.Select(value =>
                expressionFunc.Invoke(
                    Expression.MakeMemberAccess(parameter, propertyInfo),
                    BuildNullableConstantExpression<T>(value, propertyInfo))).ToList();
            ...
            string[] extractedValues, Func<MemberExpression, Expression, Expression> expressionFunc)
        {
            ...
            var toPredicate =
                typeof(ResponseCollectionFilterExtensions)
            ...
            string[] extractedValues)
        {
            ...
            extractedValues.ToList().ForEach(splitValue =>
            {
                var value = Convert(splitValue, property);
                list.Add(value);
            });
        ...
        private static Expression BuildFullTextSearchExpression<T>(ParameterExpression parameter, string[] extractedValues,
           Func<MemberExpression, Expression, Expression> expressionFunc)
        {
            var stringProperties = GetStringProperties<T>();
            if (!stringProperties.Any())
            ...
            var expressions = extractedValues.SelectMany(extractedValue =>
                stringProperties.Select(stringProperty => expressionFunc.Invoke(Expression.MakeMemberAccess(parameter, stringProperty), BuildConstantExpression(extractedValue, stringProperty))).ToList()
            ).ToList(); 
            //var expressions = stringProperties.Select(stringProperty =>
            //    expressionFunc.Invoke(Expression.MakeMemberAccess(parameter, stringProperty), BuildConstantExpression(extractedValues, stringProperty))).ToList();

            return Combine(expressions, Expression.Or);
        }

        public static Expression BuildFullTextSearchExpressionForTheSelectedFields<T>(ParameterExpression parameter, string extractedValue,
            string operatorCode, List<string> fields)
        {
            var expressionFunc = Operators.GetExpressionFunc(operatorCode);
            var stringProperties = GetStringProperties<T>().Where(s => fields.Contains(s.Name.ToLower()));
            if (!stringProperties.Any())
                throw new Exception($"Type \"{nameof(T)}\" does not have string properties for full text search");
        ...
        public static Expression BuildFullTextSearchExpressionForTheSelectedFields<T>(ParameterExpression parameter, string[] extractedValues,
            string operatorCode, List<string> fields)
        {
            var expressionFunc = Operators.GetExpressionFunc(operatorCode);
            var stringProperties = GetStringProperties<T>().Where(s => fields.Contains(s.Name.ToLower()));
            ...
            var expressions = extractedValues.SelectMany(extractedValue =>
                stringProperties.Select(stringProperty =>
                    expressionFunc.Invoke(Expression.MakeMemberAccess(parameter, stringProperty), BuildConstantExpression(extractedValue.ToLower(), stringProperty))).ToList()
            ).ToList();
                // stringProperties.Select(stringProperty =>
                //expressionFunc.Invoke(Expression.MakeMemberAccess(parameter, stringProperty), BuildConstantExpression(extractedValue.ToLower(), stringProperty))).ToList();
        ...
        private static Expression BuildNullableConstantExpression<T>(string extractedValue, PropertyInfo property)
        {
            ...
            if (IsNullable(propertyType))
                return Expression.Convert(BuildConstantExpression(extractedValue.ToLower(), property),
                    propertyType);

            return BuildConstantExpression(extractedValue.ToLower(), property);
        }

        private static Expression BuildNullableConstantExpression<T>(string[] extractedValues, PropertyInfo property)
        {
            var propertyType = GetPropertyType(property);

            if (IsNullable(propertyType))
            {
                //return Expression.Convert(BuildConstantExpression(extractedValue.ToLower(), property), propertyType);
                var expressionsEmpty = extractedValues.Select(stringProperty => Expression.Convert(BuildConstantExpression(stringProperty.ToLower(), property), propertyType)).ToList();
                return Combine(expressionsEmpty, Expression.OrElse);
            }
                
            //return BuildConstantExpression(extractedValue.ToLower(), property);

            var expressions = extractedValues.Select(stringProperty => BuildConstantExpression(stringProperty.ToLower(), property)).ToList();
            return Combine(expressions, Expression.OrElse);
        }
        ...
        private static Expression Combine(List<UnaryExpression> expressions,
            Func<Expression, Expression, Expression> combineFunc)
        {
            Expression result = null;
            foreach (var expression in expressions)
