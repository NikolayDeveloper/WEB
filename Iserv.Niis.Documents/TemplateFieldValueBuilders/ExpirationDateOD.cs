...
{
    [TemplateFieldName(TemplateFieldName.ExpirationDateOD)]
    public class ExpirationDateOD: TemplateFieldValueBase
    {
        public ExpirationDateOD(IExecutor executor) : base(executor)
        {
        }
            ...
            switch (ownerType)
            {
                case Owner.Type.Request:
                    ...
                    var requestProtectionDoc = Executor.GetQuery<GetProtectionDocByRequestIdQuery>()
                        ...
                        .FirstOrDefault();
                    return requestProtectionDoc?.MaintainDate?.ToTemplateDateFormat() ?? string.Empty;
                case Owner.Type.ProtectionDoc:
                    var protectionDoc = Executor.GetQuery<GetProtectionDocByIdQuery>()
                        .Process(q => q.Execute((int)parameters["RequestId"]));
                    return protectionDoc?.MaintainDate?.ToTemplateDateFormat() ?? string.Empty;
                default:
                    return string.Empty;
            }
        }
