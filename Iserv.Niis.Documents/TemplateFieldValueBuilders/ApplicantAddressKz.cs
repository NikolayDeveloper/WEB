...
{
	[TemplateFieldName(TemplateFieldName.ApplicantAddressKz)]
	internal class ApplicantAddressKz : TemplateFieldValueBase
	{
		public ApplicantAddressKz(IExecutor executor) : base(executor)
		{
		}
		...
		{
            //ToDo: Нужна реализация локализации адресов. На данный момент метод полная копия ApplicantAddress
		    ...
		    switch (ownerType)
		    {
		        case Owner.Type.Request:
		            ...
		            var declarantAddresses = request
		                ...
		                .Select(s => $"{s.Customer.Address} {s.Customer?.Country?.NameKz ?? string.Empty}")
		                .ToArray();
