...
{
    [TemplateFieldName(TemplateFieldName.DivisionHeadPost)]
    public class DivisionHeadPost: TemplateFieldValueBase
    {
        public DivisionHeadPost(IExecutor executor) : base(executor)
        {
        }
        ...
        protected override dynamic GetInternal(Dictionary<string, object> parameters)
        {
            ...
            return document?.Workflows?.FirstOrDefault(w => w.CurrentStage.Code == RouteStageCodes.DocumentOutgoing_02_1_1)
                       ?.DocumentUserSignature?.User?.Position?.NameRu ?? string.Empty;
        }
    }
}
